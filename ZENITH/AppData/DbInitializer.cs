...
public static class DbInitializer
{
    ...
    { "Võ Thuật Tổng Hợp", "~/image/categoryImage/sport/boxing.svg" }, // Sport Cha
    ...
    public static async Task Initialize(IServiceProvider serviceProvider)
    {
        ...
        // Đảm bảo CSDL được áp dụng Migration (nếu chưa có)
        ...
        // Luôn chạy: chỉ thêm nếu chưa tồn tại, an toàn khi chạy lại nhiều lần
        // ====================================================================
        {
            ...
            // --- 3.1 TẠO/ĐẢM BẢO CATEGORY ---
            // Tải category hiện có để tránh truy vấn lặp
            ...
            foreach (var item in sportData)
            {
                // Bảo đảm sport cha tồn tại trước
                Sport? parent = null;
                if (!string.IsNullOrEmpty(item.ParentName))
                {
                    ...
                    if (parent == null)
                    {
                        // Nếu sport cha chưa tồn tại, tạo mới nhanh
                        parent = new Sport
                        {
            ...
            // Tải liên kết hiện có để kiểm tra nhanh
